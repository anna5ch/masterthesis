...
parse_tracking_file <- function(tracking_file) {
  ...
  y_true <- ifelse(match_status %in% c("=", "c"), 1, 0)
...
extract_sample_tool <- function(filename) {
  ...
  tracking_pattern <- "only_new_(.*)\\.(.*)\\.tracking"
  feature_matrix_pattern <- "(?:salmon|oarfish)_(.*)\\.(.*)_output_feature_matrix_only_new\\.tsv"
  ...
  for (sample in sample_names) {
    ...
    for (i in seq_along(sample_tracking_files)) {
      ...
      for (feature in features) {
        if (feature != "label" && feature != "y_true" && feature != "gc_content" && feature != "exonic_length") {
          if (length(unique(merged_data$y_true)) == 2) {
            if (tracking_info$tool == "flames"){
              print(paste("Adding ROC plot for tool:", tracking_info$tool, "Feature:", feature))
              print(paste("Adding PR plot for tool:", tracking_info$tool, "Feature:", feature))
            }
            ...
            tool_plot <- tool_plot +
              geom_path(data = roc_data, aes(x = fpr, y = tpr, color = feature), linewidth = 0.5) +
              labs(
                title = paste("ROC Curve for Tool:", tracking_info$tool),
                x = "False Positive Rate", y = "True Positive Rate"
              ) + ylim(0, 1) + xlim(0, 1)
            write.table(roc_data,
              file = paste0("results/visualize/only_new/", tracking_info$tool, "_", feature, "_", sample, ".tsv"),
              sep = ",", row.names = FALSE
            )
            ...
            write.table(pr_data,
              file = paste0("results/visualize/only_new/PrecRec_", tracking_info$tool, "_", feature, "_", sample, ".tsv"),
              sep = ",", row.names = FALSE
            )
    ...
    output_file <- paste0("results/visualize/only_new/tpr_fpr_plot_", sample, "_only_new.png")
    ggsave(output_file, do.call(grid.arrange, c(plots, ncol = 2)), width = width, height = height)

    pr_output_file <- paste0("results/visualize/only_new/prec_recall_plot_", sample, "_only_new.png")
